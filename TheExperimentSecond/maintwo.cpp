...
typedef struct
{
    BYTE type; //8 位类型字段
    BYTE code; //8 位代码字段
    USHORT cksum; //16 位校验和WSOCK32.LIB
    USHORT id; //16 位标识符
    USHORT seq; //16 位序列号
} ICMP_HEADER;
...
int main()
{
    ...
    for(int i = startip; i <= endip; i++)
    {
        ...
        //查看String 与 char的转换的结果
        //cout << change(ipdata) << endl;
        u_long ulDestIP0 = inet_addr(change(ipdata));
